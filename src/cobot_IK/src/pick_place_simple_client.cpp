...
void pre_pick(moveit::planning_interface::MoveGroupInterface& move_group)
{
    ...
    // pre_pick_position.position.x = 1;
    // pre_pick_position.position.y = 0.15;
    // pre_pick_position.position.z = 0.5;

    ros::ServiceClient client_pos = nh.serviceClient<position_tracker::GetPosition>("/get_position");
    position_tracker::GetPosition service;
    service.request.get_position = true;

    if (client_pos.call(service))
    {
        pre_pick_position.position.x = service.response.x_position/1000;
        pre_pick_position.position.y = service.response.y_position/1000 + 0.15;
        pre_pick_position.position.z = service.response.z_position/1000;
    ...
    move_group.setPoseTarget(pre_pick_position);
...
void pick(moveit::planning_interface::MoveGroupInterface& move_group)
{
    ...
    if (client_pos.call(service))
    {
        pick_position.position.x = service.response.x_position/1000;
        pick_position.position.y = service.response.y_position/1000 + 0.05;
        pick_position.position.z = service.response.z_position/1000;
